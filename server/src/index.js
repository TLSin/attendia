...
app.post('/api/register', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Registration Complete", admin: adminData });
    } catch (error) {
        res.send({ error });
    }
...
app.post('/api/login',async (req, res) => {
    let { data: admin, error } = await supabase.from('admin').select('username');
...
app.listen(PORT, () => {
    console.log(`Hello World, were running on port ${PORT}`);
});